...
export default function AddStock() {
  ...
  const calculateColorStock = (c) => {
    if (!c.stock) return 0;
    return Object.values(c.stock).reduce(
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setProducts(data);
        // Auto-select product if navigated with a product ID param
        if (paramId) {
          const match = data.find(
            (p) =>
              p.id === paramId || p.productId === paramId || p._id === paramId,
          );
          if (match) {
        ...
        if (p.productType === "Bangles") {
          if (p.count === "SingleColor" && color && size) {
            newP.colors = newP.colors.map((c) => {
              ...
              const stocks = Object.values(stock).reduce(
                (sum, v) => sum + v,
                0,
              );
    ...
    <div className="max-w-6xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-lg border border-gray-200">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500">
                        {prod.productId} • {prod.productType}
                      </div>
                    </div>
      ...
      {selectedProduct && (
        <div className="mb-6 p-4 border border-gray-300 rounded-lg">
          ...
          {selectedProduct.productType === "Bangles" && (
            <>
              <p className="mb-2 text-sm text-gray-700">
                Count Type: {selectedProduct.count}
              </p>
              ...
              {selectedProduct.count === "MultiColor" && (
                <div className="flex flex-col sm:flex-row items-center gap-3 mt-4">
                  ...
                  <input
                    ...
                    placeholder="Add"
                    onChange={(e) =>
                      handleStockChange(selectedProduct.id, e.target.value)
                    }
          ...
          {/* Sarees & Jewels */}
          {(selectedProduct.productType === "Sarees" ||
            selectedProduct.productType === "Jewels") && (
          ...
          <button
            ...
            className="ml-auto flex mt-5 justify-center items-center px-4 py-2 bg-primary text-white rounded cursor-pointer"
          >
            Save Stock
          </button>
        </div>
