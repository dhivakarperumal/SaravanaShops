...
const Category = ({ onOpenModal }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const data = snapshot.docs.map((doc) => ({
        ...
        docId: doc.id,
        id: doc.id,
        sellingprice: Number(doc.data().sellingprice) || 0, // ✅ convert to number
      }));
  ...
  // On first load, apply category/subcategory filter from Navbar
  useEffect(() => {
    if (location.state) {
      ...
      if (filterType === "category") {
        ...
        setFilters((prev) => ({ ...prev, subcategory: filterValue }));
      }
    }
  }, [location.state]);
  ...
  subtitle={
    <>
      ...
      {!filters.category && !filters.subcategory && (
        <span className="text-lg font-semibold text-white">All Products</span>
      )}
    </>
  }
/>
